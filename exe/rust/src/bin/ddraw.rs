...
use windows_sys::Win32::{
    ...
    UI::WindowsAndMessaging::{
        CW_USEDEFAULT, CreateWindowExA, DefWindowProcA, DispatchMessageA, GetMessageA, MSG,
        RegisterClassA, TranslateMessage, WNDCLASSA, WS_OVERLAPPED,
    },
};
...
extern "system" fn wndproc(hwnd: HWND, msg: u32, wparam: usize, lparam: isize) -> LRESULT {
    ...
    let ret = unsafe { DefWindowProcA(hwnd, msg, wparam, lparam) };
    println!(
        "}} msg: {:x} {:?}",
        msg,
        windows_message_to_str(msg).unwrap()
...
pub extern "C" fn mainCRTStartup() {
    unsafe {
        ...
        (*(*ddraw).lpVtbl).CreateSurface.unwrap()(
            ddraw,
            &mut desc,
            &mut surf,
            core::ptr::null_mut(),
