...
function tickSpec(start2, stop, count) {
  const step = (stop - start2) / Math.max(0, count),
...
function ticks(start2, stop, count) {
  ...
  const reverse = stop < start2,
    [i1, i2, inc] = reverse ? tickSpec(stop, start2, count) : tickSpec(start2, stop, count);
  if (!(i2 >= i1)) return [];
...
function tickStep(start2, stop, count) {
  stop = +stop, start2 = +start2, count = +count;
  const reverse = stop < start2,
    inc = reverse ? tickIncrement(stop, start2, count) : tickIncrement(start2, stop, count);
...
function axis(orient, scale) {
  var tickArguments = [],
    ...
    x2 = orient === left || orient === right ? 'x' : 'y',
    transform2 = orient === top || orient === bottom ? translateX : translateY;
  function axis2(context) {
    var values = tickValues == null
        ? scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain()
        : tickValues,
      format2 = tickFormat2 == null
        ? scale.tickFormat ? scale.tickFormat.apply(scale, tickArguments) : identity_default
        : tickFormat2,
      spacing = Math.max(tickSizeInner, 0) + tickPadding,
      range = scale.range(),
      range0 = +range[0] + offset,
      range1 = +range[range.length - 1] + offset,
      position = (scale.bandwidth ? center : number2)(scale.copy(), offset),
    ...
    text = text.merge(
      tickEnter.append('text').attr('fill', 'currentColor').attr(x2, k * spacing).attr(
        'dy',
...
define_default(
  ...
  extend(Color, {
    ...
    displayable() {
      return -0.5 <= this.r && this.r < 255.5 && (-0.5 <= this.g && this.g < 255.5)
        && (-0.5 <= this.b && this.b < 255.5) && (0 <= this.opacity && this.opacity <= 1);
    },
    hex: rgb_formatHex,
    // Deprecated! Use color.formatHex.
...
function basis(t12, v0, v1, v2, v3) {
  ...
  return ((1 - 3 * t12 + 3 * t2 - t3) * v0 + (4 - 6 * t2 + 3 * t3) * v1 + (1 + 3 * t12 + 3 * t2 - 3 * t3) * v2
    + t3 * v3) / 6;
}
...
function value_default(a, b) {
  ...
  return b == null || t === 'boolean'
    ...
    : (t === 'number'
      ? number_default
      : t === 'string'
      ? (c = color(b)) ? (b = c, rgb_default) : string_default
      : b instanceof color
      ? rgb_default
      : b instanceof Date
      ? date_default
...
var setFrame = typeof window === 'object' && window.requestAnimationFrame
...
var Path = class {
  ...
  arcTo(x1, y1, x2, y2, r) {
    ...
    if (this._x1 === null) {
      ...
      this._append`A${r},${r},0,0,${+(y01 * x20 > x01 * y20)},${this._x1 = x1 + t21 * x21},${this._y1 = y1
        + t21 * y21}`;
    }
  }
  arc(x2, y2, r, a0, a1, ccw) {
    ...
    if (da > tauEpsilon) {
      ...
      this._append`A${r},${r},0,${+(da >= pi)},${cw},${this._x1 = x2 + r * Math.cos(a1)},${this._y1 = y2
        + r * Math.sin(a1)}`;
    }
  }
  rect(x2, y2, w, h) {
...
FormatSpecifier.prototype.toString = function() {
  return this.fill + this.align + this.sign + this.symbol + (this.zero ? '0' : '')
    ...
    + (this.precision === void 0 ? '' : '.' + Math.max(0, this.precision | 0)) + (this.trim ? '~' : '') + this.type;
};
...
function locale_default(locale3) {
  var group = locale3.grouping === void 0 || locale3.thousands === void 0
      ? identity_default2
      : formatGroup_default(map.call(locale3.grouping, Number), locale3.thousands + ''),
    ...
    numerals = locale3.numerals === void 0
      ? identity_default2
      : formatNumerals_default(map.call(locale3.numerals, String)),
  ...
  function newFormat(specifier) {
    ...
    function format2(value) {
      ...
      if (type2 === 'c') {
        ...
        valuePrefix = (valueNegative ? sign === '(' ? sign : minus : sign === '-' || sign === '(' ? '' : sign)
          + valuePrefix;
        valueSuffix = (type2 === 's' ? prefixes[8 + prefixExponent / 3] : '') + valueSuffix
          + (valueNegative && sign === '(' ? ')' : '');
        if (maybeSuffix) {
          i = -1, n = value.length;
          while (++i < n) {
      ...
      switch (align) {
        ...
        case '=':
          ...
          break;
        case '^':
          value = padding.slice(0, length = padding.length >> 1) + valuePrefix + value + valueSuffix
...
function transformer() {
  ...
  function scale(x2) {
    return x2 == null || isNaN(x2 = +x2)
      ? unknown
      : (output || (output = piecewise(domain.map(transform2), range, interpolate)))(transform2(clamp(x2)));
  }
...
function timeInterval(floori, offseti, count, field) {
  function interval2(date2) {
    return floori(date2 = arguments.length === 0 ? /* @__PURE__ */ new Date() : /* @__PURE__ */ new Date(+date2)),
      date2;
  }
  ...
  interval2.range = (start2, stop, step) => {
    ...
    let previous;
    do range.push(previous = /* @__PURE__ */ new Date(+start2)), offseti(start2, step), floori(start2); while (
      previous < start2 && start2 < stop
    );
    return range;
  };
  ...
  if (count) {
    ...
    interval2.every = (step) => {
      ...
      return !isFinite(step) || !(step > 0)
        ...
        : !(step > 1)
        ? interval2
        : interval2.filter(field ? (d) => field(d) % step === 0 : (d) => interval2.count(0, d) % step === 0);
    };
  }
  return interval2;
}
...
function calendar(ticks2, tickInterval, year, month, week, day, hour, minute, second2, format2) {
  ...
  function tickFormat2(date2) {
    return (second2(date2) < date2
      ...
      ? formatHour
      : month(date2) < date2
      ? week(date2) < date2 ? formatDay : formatWeek
      : year(date2) < date2
      ? formatMonth
      : formatYear2)(date2);
  }
...
function utcTime() {
  return initRange.apply(
    calendar(utcTicks, utcTickInterval, utcYear, utcMonth, utcSunday, utcDay, utcHour, utcMinute, second, utcFormat)
...
function zoom_default2() {
  ...
  function zoom(selection2) {
    selection2.property('__zoom', defaultTransform).on('wheel.zoom', wheeled, { passive: false }).on(
      'mousedown.zoom',
      mousedowned,
    ).on('dblclick.zoom', dblclicked).filter(touchable).on('touchstart.zoom', touchstarted).on(
      'touchmove.zoom',
      touchmoved,
    ).on('touchend.zoom touchcancel.zoom', touchended).style('-webkit-tap-highlight-color', 'rgba(0,0,0,0)');
  }
  zoom.transform = function(collection, transform2, point, event) {
    ...
    if (collection !== selection2) {
      ...
      selection2.interrupt().each(function() {
        gesture(this, arguments).event(event).start().zoom(
          null,
    ...
    zoom.transform(
      ...
      function() {
        var e = extent2.apply(this, arguments),
          ...
          p0 = p == null ? centroid(e) : typeof p === 'function' ? p.apply(this, arguments) : p;
        return constrain(
          identity3.translate(p0[0], p0[1]).scale(t.k).translate(
            typeof x2 === 'function' ? -x2.apply(this, arguments) : -x2,
            typeof y2 === 'function' ? -y2.apply(this, arguments) : -y2,
          ),
          e,
  ...
  function mousedowned(event, ...args) {
    ...
    var currentTarget = event.currentTarget,
      ...
      v = select_default2(event.view).on('mousemove.zoom', mousemoved, true).on('mouseup.zoom', mouseupped, true),
    ...
    function mousemoved(event2) {
      ...
      g.event(event2).zoom(
        'mouse',
        constrain(
          translate(g.that.__zoom, g.mouse[0] = pointer_default(event2, currentTarget), g.mouse[1]),
          g.extent,
          translateExtent,
        ),
      );
  ...
  function dblclicked(event, ...args) {
    if (!filter2.apply(this, arguments)) return;
    var t02 = this.__zoom,
      p0 = pointer_default(event.changedTouches ? event.changedTouches[0] : event, this),
      p1 = t02.invert(p0),
      k1 = t02.k * (event.shiftKey ? 0.5 : 2),
      t12 = constrain(translate(scale(t02, k1), p0, p1), extent2.apply(this, args), translateExtent);
    noevent_default3(event);
  ...
  zoom.extent = function(_) {
    return arguments.length
      ? (extent2 = typeof _ === 'function' ? _ : constant_default5([[+_[0][0], +_[0][1]], [+_[1][0], +_[1][1]]]), zoom)
      : extent2;
  };
...
function main() {
  ...
  const x2 = utcTime().domain([timeDay.offset(dateExtent[0], -1), dateExtent[1]]).range([
    margin.left,
    width - margin.right,
  ]);
  ...
  svg.append('g').attr('transform', `translate(${margin.left},0)`).call(
    axisLeft(y2).tickFormat((b) => prettyBytes(b)).ticks(5),
  );
  svg.append('clipPath').attr('id', 'clip').append('rect').attr('x', margin.left).attr('y', margin.top).attr(
    'width',
    width - margin.left,
  ...
  const dots = clipped.append('g').attr('fill', 'white').attr('stroke', 'steelblue').attr('stroke-width', 1).selectAll(
    'circle',
  ).data(commits.filter((d) => d.size != null)).join('circle').attr('r', 4);
  ...
  dots.on('mouseover', function(event, d) {
    ...
    tooltip.text(`${d.desc} (${d.delta >= 0 ? '+' : ''}${prettyBytes(d.delta)})`).style('left', event.pageX + 'px')
  ...
  function render(x3) {
    const line = line_default().defined((d) => d.size != null).x((d) => x3(d.date)).y((d) => y2(d.size)).curve(
      stepAfter,
    );
  ...
  render(x2);
  const zoom = zoom_default2().scaleExtent([1, 200]).extent([[margin.left, 0], [width - margin.right, height]])
    .translateExtent([[margin.left, -Infinity], [width - margin.right, Infinity]]).on('zoom', zoomed);
