...
pub fn GetFileType(sys: &dyn System, hFile: HFILE) -> u32 {
...
#[win32_derive::dllexport]
pub fn GetFileAttributesA(sys: &dyn System, lpFileName: Option<&str>) -> FileAttribute {
    ...
    let stat = match sys.host().stat(path) {
        Ok(stat) => stat,
        Err(err) => {
...
#[win32_derive::dllexport]
pub fn GetFileSize(sys: &dyn System, hFile: HFILE, lpFileSizeHigh: Option<&mut u32>) -> u32 {
    let state = get_state(sys);
    let file = match state.files.get(hFile) {
        Some(f) => f,
        None => {
