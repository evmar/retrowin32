...
async fn init_module(
    ...
    machine.process.modules.insert(hmodule, module);
...
pub async fn start_exe(machine: &mut Machine, relocate: Option<Option<u32>>) -> anyhow::Result<()> {
    let exe_name = machine.process.cmdline.exe_name();
    ...
    debug_assert!(machine.memory.process_heap.addr == 0);
    let size = 24 << 20;
    let heap = machine.memory.new_heap(size, "process heap".into());
    machine.memory.process_heap = heap;
