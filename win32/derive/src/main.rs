...
fn generate_dll(
    out_dir: &Path,
    module_name: &str,
    dllexports: &[(&syn::ItemFn, gen::DllExport)],
) -> anyhow::Result<()> {
    ...
    for (func, _) in dllexports {
        writeln!(f, ".globl _{}", func.sig.ident)?;
        writeln!(f, "_{}:", func.sig.ident)?;
        writeln!(f, "  sysenter")?;
        writeln!(f, "  ret 8")?;
    }
    ...
    writeln!(f, "EXPORTS")?;
    for (func, _) in dllexports {
        if let Some(ordinal) = func.sig.ident.to_string().parse::<usize>().ok() {
            writeln!(f, "  {} @{}", func.sig.ident, ordinal)?;
        } else {
            writeln!(f, "  {}", func.sig.ident)?;
        }
    }
